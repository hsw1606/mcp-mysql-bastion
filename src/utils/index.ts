...
/**
 * Diagnostic logging — always to stderr, never stdout.
 *
 * When the server runs over the stdio transport, stdout carries the MCP
 * protocol framing. `console.info` and `console.log` both write to stdout, so
 * a single log line there corrupts the stream and the client's handshake fails
 * with a JSON parse error. Routing every level through `console.error` keeps
 * `ENABLE_LOGGING=true` safe to use with any MCP client.
 */
