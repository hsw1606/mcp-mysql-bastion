...
- MUST 환경 변수는 `src/config/index.ts`에서만 읽고, 다른 모듈은 거기서 export한
  상수를 import한다. `test/env-reads.test.ts`가 소스를 훑어 이것을 강제한다 — 한때
  네 파일이 조용히 어기고 있었고, 그중 둘은 같은 값을 서로 다른 기본값으로 읽어
  진단 로그가 실제 접속과 다른 주소를 말했다. 주석 안의 `process.env`는 세지 않으니
...
- MUST 설정 오류 하나로 서버를 죽이지 않는다. `MYSQL_APP_SCHEMAS`의 잘못된 항목,
  깨진 catalog 파일, 못 쓸 포트 값처럼 **한 조각의 실패는 알리고 건너뛴다**.
  던지는 자리는 그것 없이는 서버가 할 일이 없는 경우뿐이고, 지금은 셋이다:
  - 읽을 수 없는 `MYSQL_ENV_FILE` — 프로필 설정이 통째로 없다
  - `MYSQL_SSL_CA`·`MYSQL_SSL_CERT`·`MYSQL_SSL_KEY`가 가리키는 파일 —
    `readSSLFile`이 모듈 스코프에서 던진다. 인증서 없이 붙는 것보다 기동을 멈추는
    쪽이 낫다
  - `src/ssh/tunnel.ts`의 `resolveTunnelConfig()` — `~/.ssh/config`에 없는 alias,
    없는 개인 키 파일, host나 user가 빠진 터널 설정. `MYSQL_SSH_ENABLED=true`인데
    터널을 못 열면 어떤 쿼리도 닿지 못한다

  이 목록을 늘리기 전에 "그 값 없이 서버가 무엇을 할 수 있는가"를 먼저 답한다.
