...
export function diagnosis(options: {
  ...
  /** 소문자 테이블 이름 → 카탈로그가 그 테이블에 대해 아는 것. */
  catalog?: Record<string, CatalogIndexFacts>;
  ...
  return {
    ...
    qualifiers,
    lookupIndexes: (name) => {
      const resolved = qualifiers.get(name.toLowerCase()) ?? { schema: null, table: name };
      return catalog[resolved.table.toLowerCase()] ?? null;
    },
  };
}
